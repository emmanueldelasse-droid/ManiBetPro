...
: ‘var(–color-text-secondary)’;
return `<div class="match-card__rec"> <span class="match-card__rec-type text-muted">${typeLabel}</span> <span class="match-card__rec-side">${sideLabel}</span> <span class="match-card__rec-odds mono">${Number(oddsDecimal).toFixed(2)}</span> <span class="match-card__rec-source text-muted">${rec.odds_source ?? ''}</span> <span class="match-card__rec-edge" style="color:${edgeColor}">+${rec.edge}%</span> </div>`;
}).join(’’);
...
el.innerHTML = `<div style=" background:linear-gradient(135deg,rgba(34,197,94,0.10),rgba(34,197,94,0.03)); border:1px solid rgba(34,197,94,0.25); border-radius:var(--radius-md); padding:12px 14px; margin-bottom:var(--space-4); cursor:pointer; " id="best-opp-card"> <div style="font-size:10px;color:var(--color-success);font-weight:700;margin-bottom:4px;letter-spacing:0.05em"> ★ MEILLEURE OPPORTUNITÉ DU JOUR </div> <div style="display:flex;justify-content:space-between;align-items:center"> <div> <div style="font-size:13px;font-weight:600"> ${bestMatch.home_team?.abbreviation} vs ${bestMatch.away_team?.abbreviation} </div> <div style="font-size:12px;color:var(--color-muted);margin-top:2px"> ${sideLabel} · ${oddsDecimal} </div> </div> <div style="text-align:right"> <div style="font-size:20px;font-weight:700;color:var(--color-success)">+${bestEdge}%</div> <div style="font-size:10px;color:var(--color-muted)">edge</div> </div> </div> </div>`;
...
container.innerHTML = `<div class="empty-state"> <div class="empty-state__icon">◎</div> <div class="empty-state__text"> Aucun match NBA aujourd'hui.<br> <span style="font-size:11px">Vérifie la connexion au Worker Cloudflare.</span> </div> </div>`;
}

function _renderError(container) {
container.innerHTML = `<div class="empty-state"> <div class="empty-state__icon">⚠</div> <div class="empty-state__text"> Erreur lors du chargement.<br> <span style="font-size:11px">Consulte la console (F12) pour plus de détails.</span> </div> </div>`;
}
